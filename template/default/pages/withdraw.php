...
?>
					...
					<!--<form method="post">-->
					  <div class="form-row">
						<div class="form-group col-md-6">
						  ...
						  <small class="form-text text-white">Ricive Amount <b id="amountfinal">1.00</b> <span id="amounymble"></span></small>
						  <div id="grey-box">
						...
						<div class="form-group col-md-6">
						  <label for="gateway"><?php echo $lang['b_180']; ?></label>
								...
								<option>Select</option>
									<?php
										$ad_packs = $db->QueryFetchArrayAll("SELECT * FROM `withdawl_config` ORDER BY `usd_convert_rate` DESC");
					  ...
					  <div class="form-row">
						<div class="form-group col-md-6">
						  <div  id="emailinport">'Enter your Email or Bank Account No'</div><p></p>
						  <div class="input-group mb-2 mr-sm-2">
							<div class="input-group-prepend"><div class="input-group-text"></div></div>
							<input type="text" class="form-control" id="idmailormob" name="idmailormob" oninput="" maxlength="250" placeholder="Email or Bank Account No">
					  	...
					  	<div class="form-group col-md-6">
						  <div  id="emailinport">'Enter Your  Phone Number'</div><p></p>
						  <div class="input-group mb-2 mr-sm-2">
							<div class="input-group-prepend"><div class="input-group-text"><i class="fa fa-phone"></i></div></div>
							<input type="text" class="form-control" id="contactnumber" name="contactnumber" oninput="" maxlength="250" placeholder=" Phone Number">
				...
				<div class="content">
					<table class="table table-striped table-sm table-responsive-sm">
						...
						<tbody class="table-primary text-dark">
							<?php
								$requests = $db->QueryFetchArrayAll("SELECT * FROM `withdrawals` WHERE `user_id`='".$data['id']."'");
								...
								else
								{
									...
									{
										echo '<tr><td>'.$request['id'].'</td><td>'.$request['coins'].'</td><td>'.$request['cash'].'</td><td>'.ucfirst($request['method']).'</td><td>'.$request['payment_info'].'</td><td>'.($request['status'] == 0 ? '<font color="blue">'.$lang['b_187'].'</font>' : ($request['status'] == 1 ? '<font color="green">'.$lang['b_188'].'</font>' : '<font color="red">'.$lang['b_189'].'</font>')).'</td></tr>';
									}
								}
							?>
						</tbody>
	...
	<?php 
	$dt_con= $db->QueryFetchArray("SELECT * FROM `coustom_config` WHERE id=1"); 
	$minm=$dt_con['c_value'];
		...
		var userEmail=$('#idmailormob').val();		
			if(!amount){
				...
				$.ajax({
					type: "POST",
					url: "system/ajax.php",
					data : 'withd='+option+'&amt='+amount+'&wop='+w_option+'&mail='+userEmail+'&contactnumber='+contactnumber,
					success: function(z) {
					console.log(z);
					if(z==='success'){
							successalert(z);
