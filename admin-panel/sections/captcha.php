		...
		<form action="" method="post" class="box">
			...
			<div class="content">
				<div class="row">
					...
					<div><select name="set[captcha_sys]" id="captcha_sys" onchange="captchaSys()"><option value="0">Disabled</option><option value="1"<?=($config['captcha_sys'] == 1 ? ' selected' : '')?>>reCaptcha</option><option value="2"<?=($config['captcha_sys'] == 2 ? ' selected' : '')?>>SolveMedia</option></select></div>
				</div>
				<span id="reCaptcha"<?=($config['captcha_sys'] == 1 ? '' : ' style="display:none"')?>>
					<div class="row">
						<label><strong>ReCaptcha Public Key</strong></label>
						<div><input type="text" name="set[recaptcha_pub]" value="<?=$config['recaptcha_pub']?>" /></div>
					</div>
					<div class="row">
						<label><strong>ReCaptcha Private Key</strong></label>
						<div><input type="text" name="set[recaptcha_sec]" value="<?=$config['recaptcha_sec']?>" /></div>
					</div>
				</span>
				<span id="solveMedia"<?=($config['captcha_sys'] == 2 ? '' : ' style="display:none"')?>>
					<div class="row">
						<label><strong>SolveMedia C-Key</strong></label>
						<div><input type="text" name="set[solvemedia_c]" value="<?=$config['solvemedia_c']?>" /></div>
					</div>
					<div class="row">
						<label><strong>SolveMedia V-Key</strong></label>
						<div><input type="text" name="set[solvemedia_v]" value="<?=$config['solvemedia_v']?>" /></div>
					</div>
					<div class="row">
						<label><strong>SolveMedia H-Key</strong></label>
						<div><input type="text" name="set[solvemedia_h]" value="<?=$config['solvemedia_h']?>" /></div>
					</div>
				</span>
			</div>
			<div class="actions">
