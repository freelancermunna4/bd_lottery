...
$coins_users = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `users` WHERE (`activate`='0' AND `disabled`!='1') AND `coins`<'10'");
...
$subscribed_users = $db->QueryFetchArray("SELECT COUNT(*) AS `total` FROM `users` WHERE (`activate`='0' AND `disabled`!='1') AND `newsletter`='1'");
?>
		...
		<form name="news" action="javascript:send()" class="box">
			...
			<div class="content">
				<div class="row">
					...
					<div><select style="width:100%" name="receivers" id="receivers" onchange="getval(this);"><option value="0">All users (<?=number_format($all_users['total'])?>)</option><option value="7">Selected Country</option><option value="3">Subscribed to Newsletter (<?=number_format($subscribed_users['total'])?>)</option><option value="4">Users with under 10 coins (<?=number_format($coins_users['total'])?>)</option><option value="5">VIP Users (<?=number_format($vip_users['total'])?>)</option><option value="1">Users active in past 7 days (<?=number_format($active_users['total'])?>)</option><option value="2">Users inactive in past 7 days (<?=number_format($inactive_users['total'])?>)</option><option value="6">Preview Email (will be sent to: <?=$config['site_email']?>)</option></select></div>
				</div>
