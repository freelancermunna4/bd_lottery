<?php
	...
	if(isset($_GET['del']) && is_numeric($_GET['del']))
	{
		...
		$db->Query("DELETE FROM `withdawl_config` WHERE `id`='".$del."'");
	}
	elseif(isset($_GET['edit']))
	{
		$edit = $db->EscapeString($_GET['edit']);
		$pack = $db->QueryFetchArray("SELECT * FROM `withdawl_config` WHERE `id`='".$edit."'");
		if(isset($_POST['submit']))
		{
			...
			$db->Query("UPDATE `withdawl_config` SET `name`='".$name."', `curency`='".$curency."', `payment_discription`='".$discription."' WHERE `id`='".$edit."'");
	...
	else
	{
		if(isset($_POST['add_pack']))
		{
			...
			if($name != '' && $curency !=''&& $discription !=''){
				$db->Query("INSERT INTO `withdawl_config` (name, curency, payment_discription) VALUES('".$name."', '".$curency."', '".$discription."')");
...
<section id="content" class="container_12 clearfix ui-sortable"><?=$message?>
	<h1 class="grid_12">All Withdrawl Bank Accaunt</h1>
	<div class="grid_8">
		<div class="box">
			<table class="styled">
				<thead>
				...
				<tbody>
<?php
	$packs = $db->QueryFetchArrayAll("SELECT * FROM `withdawl_config` ORDER BY `id` ASC");
...
?>	
					<tr>
						...
						<td class="center">
							<a href="index.php?x=withdraw&edit=<?=$pack['id']?>" class="button small grey tooltip" data-gravity=s title="Edit"><i class="icon-pencil"></i></a>
							<a href="index.php?x=withdraw&del=<?=$pack['id']?>" class="button small grey tooltip" data-gravity=s title="Remove"><i class="icon-remove"></i></a>
						</td>
					 </tr>
<?php } ?>
				</tbody>
